...
class TerminalStatModel extends Model
{
    ...
    {
        $sql = 'select  a.VAN_ID, a.CAT_MODEL_ID, e.VAN_NM,  a.reg_dt, a.tw_count as tw_count, b.cur_count, c.init_count, d.stop_count  
        ...
        left join (SELECT VAN_ID, CAT_MODEL_ID, count(*) as cur_count FROM TW_CAT_LIST 
            where STATUS = "U" 
            group by  CAT_MODEL_ID, VAN_ID) b on b.VAN_ID = a.VAN_ID and b.CAT_MODEL_ID = a.CAT_MODEL_ID       
        left join (SELECT VAN_ID, CAT_MODEL_ID, count(*) as init_count FROM TW_CAT_LIST 
            where STATUS = "A" 
            group by  CAT_MODEL_ID, VAN_ID) c on c.VAN_ID = a.VAN_ID and c.CAT_MODEL_ID = a.CAT_MODEL_ID  
        left join (SELECT VAN_ID, CAT_MODEL_ID, count(*) as stop_count FROM TW_CAT_LIST 
            where STATUS = "ST" 
            group by  CAT_MODEL_ID, VAN_ID) d on d.VAN_ID = a.VAN_ID and d.CAT_MODEL_ID = a.CAT_MODEL_ID                                                           
        left join TW_VAN_INFO e on e.VAN_ID = a.VAN_ID
    ...
    public function getTerminalVanInfoList_bak()
    {        
    ...
    // terminal/stat/list?page=1&page_count=20&search_start_dt=2022-09-15&search_end_dt=2022-09-15&van_id=V003
    // version이 있는 경우 SW count숫자 셈
    ...
    public function getTerminalUseInfoList_bak($cur_page, $page_count, $van_id, $search_start_dt, $search_end_dt)
    {        
        $sql = 'SELECT a.VAN_ID, a.CAT_MODEL_ID, c.VAN_NM, a.tw_count, count(b.CAT_MODEL_ID) as sw_count, a.reg_dt
                FROM (SELECT VAN_ID, CAT_MODEL_ID, count(CAT_MODEL_ID) as tw_count , max(REG_DT) as reg_dt FROM TW_CAT_LIST 
                    Where CASE WHEN :van_id: = "" THEN true ELSE van_id=:van_id: END
